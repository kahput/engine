		...
		if (vulkan_renderer_frame_begin(state.context, state.display.physical_width,
			...
			vulkan_renderer_draw_list_begin(state.context, main_pass);
			{
				...
				for (uint32_t index = 0; index < countof(lights); ++index) {
					...
					Matrix4f transform = mat4f_translated(*(float3 *)vec4f_elements(&lights[index].as.position));
...
void editor_update(float dt) {
	...
	float3 target_position = vec3f_normalize(
	...
	// Calc right
	float3 camera_right = vec3f_cross(target_position, state.editor_camera.up);
	camera_right = vec3f_normalize(camera_right);

	// Calc down
	float3 camera_down = vec3f_negate(state.editor_camera.up);
