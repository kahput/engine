...
SMesh mesh_source_cube_face_create(Arena *arena, float x, float y, float z, uint8_t orientation) {
	SMesh rv = {
...
void mesh_source_list_push(Arena *arena, SMeshList *list, SMesh source) {
	SMeshNode *node = arena_push_struct(arena, SMeshNode);
	node->source = source;
	node->next = NULL;
...
SMesh mesh_source_list_flatten(Arena *arena, SMeshList *list) {
	SMesh rv = { 0 };
	if (list->count == 0)
		return rv;
